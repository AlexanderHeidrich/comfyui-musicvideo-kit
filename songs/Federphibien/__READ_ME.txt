...
RENDERING THIS FOLDER IN COMFYUI
  ...
  THREE STARTING WORKFLOWS ARE IN THIS FOLDER
    ...
    them. All three are API-format graphs - open them with Workflow -> Open.
      ...
      __wf_2_folder.json    the one you want. MV Song Folder reads this folder
                            and drives everything.
      __wf_3_pipeline.json  MV Build Song runs ./mvkit on the host first, then
                            the same. For when you want one button.

    First, once: copy (or symlink) comfyui/custom_nodes/mv_h3_nodes.py into your
    ...
    __wf_2_folder.json is wired like this:
      ...
      MV Song Folder ──► prompt      ──► H3 ref2vid ──► Save Video
       (song_path)   ──► frames      ──►
